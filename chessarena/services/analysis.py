...
def _probe_positions(binary: Path, fens: list[str]) -> list[dict]:
    ...
    converted to White perspective."""
    ...
    try:
        _write_and_wait(proc, "uci", "uciok")
        _write_and_wait(proc, "isready", "readyok")
        for name, value in (
        ...
        for fen in fens:
            ...
            for line in _read_until(proc, "bestmove"):
...
def _write_and_wait(proc: subprocess.Popen, cmd: str, terminator: str) -> None:
    proc.stdin.write(cmd + "\n")
    proc.stdin.flush()
    for _ in _read_until(proc, terminator):
        pass


def _read_until(proc: subprocess.Popen, terminator: str) -> list[str]:
    lines: list[str] = []
    while True:

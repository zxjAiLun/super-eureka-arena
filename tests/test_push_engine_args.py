...
class TestRemoteConstruction:
    ...
    def test_plain_args_emit_a_single_flag_not_one_per_value(self):
        """One flag per value would make the remote parser keep only the last."""
        rendered = push_engine.preset_args_for_remote(["a", "b", "c"])
        assert rendered == ["--command-args", "a", "b", "c"]
        assert rendered.count("--command-args") == 1

    def test_empty_args_emit_nothing(self):
        assert push_engine.preset_args_for_remote([]) == []

    def test_unsupported_leading_dash_fails_closed(self):
        with pytest.raises(SystemExit) as excinfo:
            push_engine.preset_args_for_remote(["--hash", "16"])
        assert "leading-dash" in str(excinfo.value)
